...
import { HttpQuick as HttpQuickBase } from './httpquick';
    ...
    try {
      ...
      try {
        if (req.responseType == 'text' && req.dataType == 'json') {
          ...
          res.body = Buffer.isBuffer(response.data) ? response.data.toString('utf8') : typeof response.data == 'string' ? response.data : response.data;
        } else {
