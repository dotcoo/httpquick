...
class HttpQuickBase {
  ...
  // 填充请求行
  fillUrl(req) {
    req.fullUrl = (/^https?:\/\//.test(req.url) ? req.url : req.baseUrl + req.url) + (req.query && Object.keys(req.query).length ? '?' + this.obj2query(req.query) : '');
  }

  // 填充请求头
  fillHeaders(req) {
    if (req.responseType == 'text' && req.dataType == 'json') { req.headers.Accept = 'application/json'; }
    else if (req.responseType == 'text' && req.dataType == 'xml') { req.headers.Accept = 'application/xml'; }
    else if (req.responseType == 'text') { req.headers.Accept = 'text/plain'; }
    else if (req.responseType == 'arraybuffer' || req.responseType == 'blob') { req.headers.Accept = 'application/octet-stream'; }
  }
  ...
  // 超时错误处理
  onTimeoutError = function(req, res, err = null) {
    console.error('http timeout', err);
    res.status = -4;
    res.statusText = 'TIMEOUT';
    if (req.responseType == 'text' && req.dataType == 'json') {
      res.body = { errno: -1, errmsg: (res.response.status ?? res.status) + ', 网络请求超时!' };
    } else {
      res.body = '网络请求超时!';
    }
  };
  ...
  // 网络错误处理
  onNetworkError(req, res, err = null) {
    console.error('http error', err);
    res.status = -1;
    res.statusText = 'NETWORK FAIL';
    if (req.responseType == 'text' && req.dataType == 'json') {
      res.body = { errno: -2, errmsg: (res.response.status ?? res.status) + ', 网络请求错误!' };
    } else {
      res.body = '网络请求错误!';
    }
  }
  ...
  globalMethods() {
    ...
    globalThis.httpDelete = this.delete.bind(this);
    globalThis.httpPatch = this.patch.bind(this);
    globalThis.httpHead = this.head.bind(this);
    globalThis.httpUpload = this.upload.bind(this);
    globalThis.httpDownload = this.download.bind(this);
    globalThis.httpRequest = this.request.bind(this);
  }

  mathMethods() {
    Math.get = this.get.bind(this);
    Math.post = this.post.bind(this);
    Math.put = this.put.bind(this);
    Math.delete = this.delete.bind(this);
    Math.patch = this.patch.bind(this);
    Math.head = this.head.bind(this);
    Math.upload = this.upload.bind(this);
    Math.download = this.download.bind(this);
    Math.request = this.request.bind(this);
  }
  ...
  mathMethods() {
    ...
    Math.post = this.post.bind(this);
    Math.put = this.put.bind(this);
    Math.delete = this.del.bind(this);
    Math.upload = this.upload.bind(this);
    Math.download = this.download.bind(this);
  }
}
